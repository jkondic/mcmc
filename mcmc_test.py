...
def sos_cost(x):
    vec = jnp.ones((2,2))
    return jnp.sum(jnp.power(x - vec, 2)) # sum of squares cost
...
target = Boltzmann(shape=(2,2), cost=sos_cost, sampler='hmc', beta=1) # select 'mh' or 'hmc' or 'mala' as sampler
samples = target.sample(n=10000)

### plot the samples ###
x_axis = np.arange(-5, 5, 0.01)
# get the histogram for marginal at index [0,0]
plt.hist(samples[:,0,0], bins=10, density=True)
